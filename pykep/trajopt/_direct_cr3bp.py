...
class direct_cr3bp:
    ...
    def __init__(
        ...
        self.ta_cr3bp = _pk.ta.get_cr3bp(1e-16)
        self.ta_cr3bp.pars[0] = mu
    ...
    def _set_leg_from_x(self, x):
        ...
        self.ta_cr3bp.time = 0.0
        self.ta_cr3bp.state[:3] = self.pls[0]
        self.ta_cr3bp.state[3:6] = self.pls[1]
        rs = self.ta_cr3bp.state[:3].copy()
        vs = self.ta_cr3bp.state[3:6].copy()
        # Propagate target state
        self.ta_cr3bp.time = 0.0
        self.ta_cr3bp.state[:3] = self.plf[0]
        self.ta_cr3bp.state[3:6] = self.plf[1]
        self.ta_cr3bp.propagate_until(x[0] + x[-1])
        rf = self.ta_cr3bp.state[:3].copy()
        vf = self.ta_cr3bp.state[3:6].copy()
    ...
    def gradient(self, x):
        ...
        for i in range(6):
            ...
            dyn_xs = self.cf_dyn(_np.concatenate([rs, vs]).reshape(-1).tolist(), pars=[self.leg.mu])
            dyn_xf = self.cf_dyn(_np.concatenate([rf, vf]).reshape(-1).tolist(), pars=[self.leg.mu])
            
            # dyn_xs = self.cf_dyn(_np.concatenate([rs, vs, _np.array([self.leg.ms])]).reshape(-1).tolist(), pars=[self.leg.mu, self.leg.veff, self.leg.throttles[0], self.leg.throttles[1], self.leg.throttles[2]])
            # dyn_xf = self.cf_dyn(_np.concatenate([rf, vf, _np.array([self.leg.mf])]).reshape(-1).tolist(), pars=[self.leg.mu, self.leg.veff, self.leg.throttles[-3], self.leg.throttles[-2], self.leg.throttles[-1]])
    ...
    def plot(
        ...
        **kwargs,
        ...
        self.ta_cr3bp.state[:3] = self.pls[0]
        self.ta_cr3bp.state[3:6] = self.pls[1]
        sol_0 = self.ta_cr3bp.propagate_grid(tgrid)
        status = sol_0[0]
        integration_0 = sol_0[5]
        # Crop time
        tgrid = tgrid[:len(integration_0[:,0])]
        if status.name != 'time_limit':
            integration_0 = _np.concatenate([integration_0, [self.ta_cr3bp.state]]) 

        # Propagate target state
        self.ta_cr3bp.time = 0.0
        self.ta_cr3bp.state[:3] = self.plf[0]
        self.ta_cr3bp.state[3:6] = self.plf[1]
        sol_f = self.ta_cr3bp.propagate_grid(tgrid)
        status = sol_f[0]
        integration_f = sol_f[5]
        # Crop time
        tgrid = tgrid[:len(integration_f[:,0])]
        if status.name != 'time_limit':
            integration_f = _np.concatenate([integration_f, [self.ta_cr3bp.state]]) 
